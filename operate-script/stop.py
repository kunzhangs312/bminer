...
def main(name):
    ...
    try:
        ...
        userid = sys.argv[2]
        id = sys.argv[3]
        str_ = {"id":id,"userid":userid,"operator_type": "stop", "status": status, "mac": mac, "time": time.time(),"program":name,"operate_name":"stop"}
        json_info = json.dumps(str_)
        # producer.produce(bytes(json_info, encoding="utf8"))
        SYN = random.randint(0, 100000000)
        response = '{"taskid":' + id + ', "action":"", "maclist":[' + mac + '], "result":' + json_info + ', "syn": ' + SYN + '}'
